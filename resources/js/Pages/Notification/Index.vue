<template>
    <h1 class="text-3xl mb-4">This is your notifications</h1>
            ...
            <div>
                <span v-if="notification.type === 'App\\Notifications\\OfferMade'">
                    ...
                    <Link :href="route('realtor.listing.show', { listing: notification.data.listing_id })"
                        class="text-indigo-600 dark:text-indigo-300"> Listing
                    </Link>
                    was made
                </span>
            </div>
            <div>
                <Link :href="route('notification.seen', { notification: notification })" method="PUT" as="button"
                    v-if="!notification.read_at" class="btn-outline text-xs font-medium uppercase">Make as
                read</Link>
            </div>
        </div>
    </section>
